...
- 在籍ステータスが `在籍中` 以外の参加者は、どのチームにも所属しない
- 在籍ステータス操作（入会/復帰/休会/退会）を起点に、チーム再編を自動実行する
  - 入会/復帰: 最少人数チームへ自動配属（同数はランダム）
  - 休会/退会: チームから自動離脱
  - 1名チーム発生: 自動合流（最少人数優先、同数はランダム）
  - 5名チーム発生: 自動分割
  - 2名以下または合流先なし: 管理者へ通知
- 課題進捗の遷移ルールは以下
  - `未着手 -> 取組中`
